...
| 操作 | 效果 |
| --- | --- |
| A 键 | 自动巡检开关（默认**已开启**，按一下可关掉） |
| S 键 | 切换停留时长：3s → 5s → 10s → 30s → 60s → 5m → 10m 循环 |
| O 键 | 切换轮播顺序：顺序（SEQ）→ 随机（RND）→ 往返（PINGPONG） |
| 点击 / 空格 / 右方向键 | 下一个画面 |
| 左方向键 | 上一个画面 |
| F 键 | 随机闪烁模式（看响应和拖影） |
| 拖动 | 画框并显示像素尺寸 |
...
**程序启动后默认就在自动巡检状态**：画面按设定的停留时长自动轮播，**永不退出、循环播放**，可以挂着跑一整晚。
切换设置时左上角会短暂显示一行状态（例如 `AUTO ON  3/8  10s  SEQ`），每次自动换画面时也会显示一下当前进度。

- 默认：**停留 10 分钟**、顺序播放（1→2→…→8→1）
- 按 A 可以关掉自动巡检，回到手动点击/空格切换
- 按 S 调停留时长，按 O 调顺序；手动点一下或按空格会立刻换画面并重新计时
- 停留时长是跨版本一致的固定档位：`3s / 5s / 10s / 30s / 60s / 5m / 10m`，从 10m 再按一下回到 3s
- 拖动测量、长按退出的过程中会暂停轮播，不会打断你正在做的测量
- 自动巡检与闪烁模式（F）互斥：开一个会关掉另一个
- 顺序模式按 1→8 循环；随机模式每次随机挑一张；往返模式走到第 8 张后掉头往回走
